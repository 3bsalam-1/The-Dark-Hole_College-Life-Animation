...
void CollegeTitleScreen() {
  ...
  // Story Title
  print("The Dark Hole", 1, 1, 1, CO_chap_fade, 300 + trans_x_chap3, 400, .2,
        .2, 5);
  print("College", 1, 1, 1, CO_title_fade, 400 - trans_x_title3, 350, .3, .3,
        3);
}
...
void CO_drawSun() {
  // Sun
  drawCircle(1250 - sun_move_left, 700, sun_r, sun_g, sun_b, 1, 1, 50);
}
...
void CO_drawHome() {
  ...
  // Top Circle
  drawCircle(1160, 420, 255, 255, 255, 1, 1, 10);
}
...
void CO_drawLights() {
  ...
  // Left Bulb
  drawCircle(350, 300, window_top_r, window_top_g, window_top_b, 1, 1, 15);

  // Right Bulb
  drawCircle(1050, 300, window_top_r, window_top_g, window_top_b, 1, 1, 15);
}
...
void CO_drawStars() {
  if (stars_are_made == false) {
    ...
    for (int i = 0; i < no_of_stars; i++)
      glVertex2f(stars_array[i][0], stars_array[i][1]); // Corrected indexing
    glEnd();
    glPointSize(1);
  }
}

void drawCollegeBoy(GLfloat tx, GLfloat ty, GLfloat sx, GLfloat sy,
...
void College() {
  ...
  drawCollegeBoy(Collegeboy_x, Collegeboy_y, .5, .5, 255, 0, 0);

  print(
      "Home -> FCISCU -> ITI -> Home -> FCISCU -> ITI -> Home -> FCISCU -> ITI",
